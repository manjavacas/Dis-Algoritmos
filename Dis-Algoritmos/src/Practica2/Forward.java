...
public class Forward {
	...
	public Cambio resolver() {
		...
		// Añado el primer cambio, el cual tendra todos los valores inicializados a 0,
		// la etapa a 1 y el cambio a devolver como el cambio restante
		cambios.add(new Cambio(-1, cambio, 0, 0, 0, null));

		// Se lleva a cabo el algoritmo hasta que hayamos recorrido todos los elementos
		// de la lista
		while (pos < cambios.size()) {
			...
			if (etapa < numMonedas) {
				...
				for (int n = 0; n <= max; n++) { // Comprobamos todas las combinaciones de monedas a meter
					...
					if (nuevoRestante >= 0) {
						// Obtenemos los valores del nuevo cambio generado si metemos tantas monedas de
						// dicha moneda
						...
						if (!cambios.contains(nuevo)) {
							cambios.add(nuevo);
						} else { // Si ya se ha generado se comprueba si es mejor que el cambio que habia antes y
									// se actualiza en caso de serlo
	...
	private boolean esMejor(Cambio nuevo, Cambio previo) {
		...
		// Es mejor si los valores acumulados son iguales pero el numero de monedas que
		// utilizamos es menor
